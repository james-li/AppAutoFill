...
1. 创建配置文件。配置文件为app名即可，这里创建navicate-premium.json文件
    ...
    2. 按实际情况和提示修改navicate-premium.json文件中的参数。如下
        ...
        ```
    3. 创建一个简单的web服务器，把navicat.json copy到服务器下，web服务器简单的代码在[web_server_simulator.py](https://gitlab.cloudbility.cn:4443/cloudsoft/cloudSoft-cpp/blob/master/AppAutoFill/web_server_simulator.py)，下载下来，在同目录创建test目录，将navicate-premium.json复制到test目录即可。然后运行下列命令
    ```bash
